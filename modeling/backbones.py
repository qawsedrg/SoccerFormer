...
class ConvTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        for idx in range(arch[2]-1):
            self.upsampple.append(nn.Conv1d(n_embd,n_embd,1))
        # init weights
    ...
    def forward(self, x, mask):
        ...
        '''
        for idx in range(len(self.branch)-2,-1,-1):
            x=out_feats[idx+1]
            y=self.upsampple[idx](out_feats[idx])
            out_feats[idx]=nn.Upsample(scale_factor=2, mode='linear')(x)+y
        '''

        return tuple(out_feats), tuple(out_masks)

@register_backbone("convTransformer_modified")
class ConvTransformerBackbone_modified(nn.Module):
    """
        A backbone that combines convolutions with transformers
    """
    def __init__(
